...
import { useCancelLeaveRequest } from '../hooks/useLeaveRequest';
...
export function LeaveHistoryTable({ requests, showCancel = false }: Props) {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-200">
            ...
            {showCancel && <th className="py-3 px-4" />}
          </tr>
        </thead>
        <tbody>
          {requests.map(r => (
            <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
              ...
              <td className="py-3 px-4">
                ...
                {r.status === 'REJECTED' && r.rejectionReason && (
                  <p className="text-xs text-gray-400 mt-0.5">{r.rejectionReason}</p>
                )}
              </td>
